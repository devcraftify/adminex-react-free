...
const STATUS_COLORS: Record<TaskStatus, { bg: string; border: string; text: string }> = {
  pending: { bg: 'bg-gray-100 dark:bg-gray-700', border: 'border-gray-300', text: 'text-gray-600 dark:text-gray-300' },
  ...
  cancelled: { bg: 'bg-gray-100 dark:bg-gray-700', border: 'border-gray-400', text: 'text-gray-500' },
...
export const GanttChart: React.FC<GanttChartProps> = ({
  ...
  <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
      ...
      <div className="flex">
        {/* Task List */}
        <div className="w-[200px] flex-shrink-0 border-r border-gray-200 dark:border-gray-700">
          {/* Header */}
          <div 
            className="px-4 py-3 bg-gray-50 dark:bg-gray-700/50 border-b border-gray-200 dark:border-gray-700 font-medium text-sm text-gray-600 dark:text-gray-300"
          ...
          >
            {ganttTasks.map((task) => (
              <div
                ...
                className={`px-4 flex items-center gap-2 border-b border-gray-100 dark:border-gray-700 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors ${
        ...
        >
          <div style={{ width: `${timelineConfig.totalDays * timelineConfig.cellWidth}px` }}>
            {/* Timeline Header */}
            <div 
              className="flex border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-700/50"
              style={{ height: `${timelineConfig.headerHeight}px` }}
            >
              {/* Month row */}
                ...
                {timelineHeaders.filter(h => {
                  ...
                  return (
                    <div
                      ...
                      className="text-xs font-medium text-gray-600 dark:text-gray-300 px-2"
                      style={{ 
                        position: 'absolute',
              ...
              <div className="flex mt-7">
                {timelineHeaders.map((header, i) => (
                  <div
                    key={i}
                    className={`flex-shrink-0 text-center text-xs border-l border-gray-200 dark:border-gray-600 ${
                      header.isToday 
            ...
            <div className="relative">
              ...
              <div className="absolute inset-0 flex">
                {timelineHeaders.map((header, i) => (
                  <div
                    key={i}
                    className={`flex-shrink-0 border-l border-gray-100 dark:border-gray-700 ${
                      header.isToday 
      ...
      {/* Legend */}
      <div className="flex items-center gap-6 px-4 py-3 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-700/30">
        ...
        <span className="text-xs font-medium text-gray-500 dark:text-gray-400 ml-4">Priority:</span>
        {Object.entries(PRIORITY_INDICATORS).map(([priority, color]) => (
          <div key={priority} className="flex items-center gap-1.5">
            <div className={`w-2 h-2 rounded-full ${color}`} />
            <span className="text-xs text-gray-600 dark:text-gray-300 capitalize">{priority}</span>
          </div>
        ))}
      </div>
    </div>
