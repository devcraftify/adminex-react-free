      ...
      <div className="max-w-7xl mx-auto relative z-10">
        <div className="text-center mb-20">
          ...
          <h2 className="text-display-section text-secondary-900 dark:text-white mb-6">
            {t('landing.pricing.title_prefix')} <span className="text-gradient">{t('landing.pricing.title_emphasis')}</span>
          </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-6xl mx-auto">
          {plans.map((plan) => (
            <div
              key={plan.nameKey}
              className={`relative rounded-[2rem] p-8 flex flex-col transition-all duration-300 ${plan.popular
                ? 'bg-white dark:bg-surface-900 shadow-2xl shadow-theme-primary/10 border-2 border-theme-primary scale-105 z-10'
                : 'bg-surface-50 dark:bg-surface-950 border border-surface-200 dark:border-surface-800 hover:border-theme-primary/30 hover:shadow-xl'
                }`}
            >
              {plan.popular && (
                <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-theme-primary text-white px-4 py-1 rounded-full text-sm font-bold shadow-lg shadow-theme-primary/20">
              ...
              <button
                className={`w-full py-4 rounded-xl font-bold transition-all duration-200 ${plan.popular
                  ? 'bg-theme-primary text-white hover:brightness-90 shadow-lg shadow-theme-primary/25 hover:shadow-theme-primary/40'
                  : 'bg-white dark:bg-surface-800 text-secondary-900 dark:text-white border border-surface-200 dark:border-surface-700 hover:bg-surface-50 dark:hover:bg-surface-700'
                  }`}
              >
                {t(plan.ctaKey)}
              </button>
